...
def keyword_matches(keyword: str, cv_text_lower: str) -> bool:
    if keyword in cv_text_lower:
        return True
    keyword_words = [w for w in keyword.split() if len(w) > 4]
    for word in keyword_words:
        stem = word[:5]
        if stem not in cv_text_lower:
            return False
    return len(keyword_words) > 0
...
def fuzzy_match_onet(onet_skill: str, cv_skills: set, cv_text_lower: str, onet_to_terms: dict) -> bool:
    ...
    for term in onet_to_terms.get(onet_skill, []):
        if keyword_matches(term, cv_text_lower):
            return True
...
def score_hard_skills(facts: dict, onet_data: dict, ats_list: list,
                      cv_text: str = "", onet_to_terms: dict = {},
                      ats_keywords_with_scores: list = []) -> dict:
    try:
        score = 0
        details = []
        ...
        top_onet_skills = aggregate_by_element(onet_data.get("skills", []), min_score=55)
        top_onet_skills |= aggregate_by_element(onet_data.get("knowledge", []), min_score=55)
        top_onet_skills |= aggregate_by_element(onet_data.get("abilities", []), min_score=55)
        ...
        if ats_keywords_with_scores:
            ats_score, ats_detail = _weighted_ats_score(ats_keywords_with_scores, cv_text_lower)
            score += ats_score
            details.extend(ats_detail)
        else:
        ...
        import traceback
        print("score_hard_skills crashed:")
        traceback.print_exc()
        return {"score": 0, "details": [f"⚠ Scoring error: {str(e)}"]}

def _weighted_ats_score(ats_keywords_with_scores: list, cv_text_lower: str) -> tuple[int, list]:
    
    details = []
    
    if not ats_keywords_with_scores:
        return 0, ["~ No ATS keyword data available"]

    total_weight = sum(kw["similarity"] for kw in ats_keywords_with_scores)
    if total_weight == 0:
        return 0, ["~ ATS weight sum is zero"]

    matched_weight = 0.0
    missing_critical = []   
    missing_moderate = []   

    for kw in ats_keywords_with_scores:
        keyword = kw["keyword"].lower()
        sim = kw["similarity"]

        if keyword_matches(keyword, cv_text_lower):
            matched_weight += sim
        else:
            if sim >= 0.75:
                missing_critical.append((keyword, sim))
            elif sim >= 0.55:
                missing_moderate.append((keyword, sim))

    coverage = matched_weight / total_weight  
    ats_score = round(coverage * 25)          

    missing_critical.sort(key=lambda x: x[1], reverse=True)
    missing_moderate.sort(key=lambda x: x[1], reverse=True)

    details.append(f"✓ ATS weighted coverage: {coverage:.0%} (+{ats_score})")

    if missing_critical:
        kws = [k for k, _ in missing_critical[:5]]
        details.append(f"  ⚠ Critical gaps (high-value keywords): {kws}")

    if missing_moderate:
        kws = [k for k, _ in missing_moderate[:5]]
        details.append(f"  ~ Moderate gaps: {kws}")
